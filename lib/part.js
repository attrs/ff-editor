...
function Part(arg) {
  if( arg && arg.__ff__ ) return arg.__ff__;
  if( !(this instanceof Part) ) return null;
  if( !arg ) arg = this.create();
  if( !context.isElement(arg) ) arg = this.create(arg);
  if( !context.isElement(arg) ) throw new TypeError('illegal argument: arg');
  arg.__ff__ = this;
  
  var dom = arg;
  var el = $(arg).addClass('ff ff-part');
